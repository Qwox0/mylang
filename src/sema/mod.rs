    ...
    while remaining_count > 0 {
        ...
        for file in cctx.files().iter().copied() {
            ...
            if stmt_range.start == stmt_range.end {
                continue; // nocheckin: bench
            }

            debug_assert!(file.scope.as_ref().u().parent.is_some());
            sema.open_scope(file.as_mut().scope.as_mut().u());
            //let stmt_range = file.as_mut().stmt_range.as_mut().u(); // nocheckin: bench move up
...
impl Sema {
    ...
    fn open_scope(&mut self, new_scope: &mut Scope) {
        ...
        if new_scope.kind == ScopeKind::File {
            println!(
                "open_scope {:p} ({:?}), cur_scope: {:p} ({:?}); new_scope.parent: {:p} ({:?})",
                new_scope,
                new_scope.kind,
                self.cur_scope,
                self.cur_scope.kind,
                new_scope.parent.unwrap(),
                new_scope.parent.unwrap().kind,
            );
        }
        */
